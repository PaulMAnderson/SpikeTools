...
function  cluster = clusterPositions(spikeStruct, clusterIDs)

if nargin < 2
    clusterIDs = unique(spikeStruct.cids);
end
...
%% Find which template matches each cluster
for clusterI = 1:length(clusterIDs)
    spikeTemplates = spikeStruct.spikeTemplates(spikeStruct.clu == clusterIDs(clusterI));
